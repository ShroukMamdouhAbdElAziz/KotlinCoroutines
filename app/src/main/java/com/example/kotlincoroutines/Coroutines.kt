...
First understand How exactly your application works, when the user launches your application
 -------------------------------------------------------------------------------------------
 1- when the user launches the application ,by default the Main thread is created( Noting that  Main Thread is the life of your application).
...
-------------------------------------------
 *like(long Operations, File download from the server ,Network Operations ,loading and displaying an Image ,heavy some database Queries) .
 * If you tried to perform these operations on the Main Thread , The main Thread will be blocked hence, the application will be freezed ( app will be hanged).
 *If the Main Thread still freezed for 5 secs or more ,You will get ANR ( application not responding) with Crash message.
    ---------------
     The Solutions
     --------------
 Solution   1# Instead of using Main Thread for these Operations, we need to use worker or background Thread for EACH Operation to run the Heavy operations. like :
 --------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 ### But Noting that there is a LIMIT of How Many BackGround THreads you can Create at a time in your App. Because creating thread is very very Expensive
 ...
 --------------------------
 When you use coroutines , you don't have to create so many threads for each operations. INSTEAD you can just have ONE background thread
